(function(){
  ...
  function open(name,data){
    const m=ensure(); let description='',levels=[];
    if(Array.isArray(data)) levels=data; else if(data){description=data.description||'';levels=data.levels||[];}
    document.getElementById('skillInfoTitle').textContent=name;
    const intro=description?`<p class="skillInfoDescription">${esc(description)}</p>`:'';
    const rows=(levels||[]).filter(x=>x&&x.effect).map(x=>`<div class="skillInfoLevel"><b>Lv${esc(x.level)}</b><span>${esc(x.effect)}</span></div>`).join('');
    document.getElementById('skillInfoBody').innerHTML=intro+(rows||'<p class="small">このスキルの効果データは登録されていません。</p>');
    m.classList.remove('hidden');
  }
  window.MHSkillPopover={open,close,button:(name,levels)=>{const payload=encodeURIComponent(JSON.stringify({name,data:levels}));return `<button type="button" class="skillInfoBtn" data-skill-info="${payload}" aria-label="${esc(name)}の効果を表示">ⓘ</button>`;}};
  document.addEventListener('click',e=>{const b=e.target instanceof Element ? e.target.closest('.skillInfoBtn[data-skill-info]') : null;if(!b)return;e.preventDefault();e.stopPropagation();try{const raw=b.getAttribute('data-skill-info')||'';const x=JSON.parse(decodeURIComponent(raw));open(x.name,x.data||[]);}catch(err){console.error('skill info popup error',err);}},true);
})();
