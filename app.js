...
function solve(){if(!state.db)return;const t=targets(),allowQ=$('allowQurious').checked;const parts=[['頭',state.db.head],['胴',state.db.chest],['腕',state.db.arms],['腰',state.db.waist],['脚',state.db.legs]];const mr=$('tier').value==='mr';const limit=Math.min(+$('limit').value||10,50);if(!Object.keys(t).length){render([],t,0,[], '目標スキルを1つ以上選択してください。');return}
  const pools=parts.map(([p,rows])=>rows.map(r=>armorRow(r,p)).filter(a=>!mr||a.rarity>=8));const n=Object.keys(t).length;const cap=n<=1?100:n<=2?140:n<=3?180:n<=4?220:260;pools.forEach(p=>{p.sort((a,b)=>scoreArmor(b,t)-scoreArmor(a,t));if(p.length>cap)p.length=cap});
