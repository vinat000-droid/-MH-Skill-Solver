...
function skillInfoForKey(k){const rows=(state.db?.skill||[]).filter(r=>keyFor(r['#スキル系統']||r['スキル系統']||r['発動スキル'])===k);const name=targetLabel(k);const detail=(window.SUNBREAK_SKILL_DETAILS||{})[name]||{};const levels=rows.map(r=>({level:parseSkillLevel(r['発動スキル']),effect:String(r['効果']||'').trim()})).filter(x=>x.effect).sort((a,b)=>a.level-b.level);const merged=levels.map(x=>({...x,summary:detail||{}}));return {name,description:detail.description||detail||'',levels:merged};}
