...
function skillInfoForKey(k){const rows=(state.db?.skill||[]).filter(r=>keyFor(r['スキル系統']||r['発動スキル'])===k);const levels=rows.map(r=>({level:parseSkillLevel(r['発動スキル']),effect:String(r['効果']||'').trim()})).filter(x=>x.effect);return levels.sort((a,b)=>a.level-b.level);}
