...
const LABEL={elementalAttack:'属性攻撃強化',elementalWeakness:'弱点特効【属性】',dragonConversion:'龍気変換',furious:'激昂',steelBlessing:'鋼殻の恩恵',dereliction:'伏魔響命',loadShells:'砲弾装填',frostcraft:'冰気錬成',guardUp:'ガード強化',embolden:'煽衛',evadeExtender:'回避距離UP'};
...
function keyFor(s){const n=norm(s);const canonical=n.replace(/\[属性\]/g,'【属性】').replace(/（属性）/g,'【属性】');return ({'回避距離UP':'evadeExtender','弱点特効【属性】':'elementalWeakness','弱点特効':'weaknessExploit','属性攻撃強化':'elementalAttack','火属性攻撃強化':'elementalAttack_fire','水属性攻撃強化':'elementalAttack_water','氷属性攻撃強化':'elementalAttack_ice','雷属性攻撃強化':'elementalAttack_thunder','龍属性攻撃強化':'elementalAttack_dragon','龍気変換':'dragonConversion','激昂':'furious','鋼殻の恩恵':'steelBlessing','伏魔響命':'dereliction','砲弾装填':'loadShells','冰気錬成':'frostcraft','ガード強化':'guardUp','煽衛':'embolden'})[canonical]||canonical}
...
function armorRow(row,part){const skills={};skillPairs(row).forEach(([k,v])=>skills[k]=(skills[k]||0)+v);return{part,name:row['#名前']||row['名前']||row['装備名']||'名称不明',slots:[+row['スロット1']||0,+row['スロット2']||0,+row['スロット3']||0],rarity:+row['レア度']||0,skills}}
...
function skillInfoForKey(k){const rows=(state.db?.skill||[]).filter(r=>keyFor(r['#スキル系統']||r['スキル系統']||r['発動スキル'])===k);const name=targetLabel(k);const detail=(window.SUNBREAK_SKILL_DETAILS||{})[name]||'';const csvDescription=rows.find(r=>String(r['基本説明']||'').trim())?.['基本説明']||'';const description=typeof detail==='string'?(detail||csvDescription):((detail.description||'')||csvDescription);const levels=rows.map(r=>({level:parseSkillLevel(r['発動スキル']),effect:String(r['効果']||'').trim()})).filter(x=>x.effect).sort((a,b)=>a.level-b.level);return {name,description,levels};}
...
function decos(db){return(db.decorations||[]).map(r=>{const effects=[];for(let i=1;i<=3;i++){const raw=r['スキル系統'+i],v=+(r['スキル値'+i]||0),k=keyFor(raw);if(k&&v)effects.push([k,v])}return{name:r['#名前']||r['名前']||r['装飾品名']||'名称不明',size:+r['スロットサイズ']||0,effects}}).filter(d=>d.size&&d.effects.length)}
