...
/* PERSISTENCE_V16 */
const SAVE_KEY='mhrsb-solver-v16-state';
const RESULT_KEY='mhrsb-solver-v16-last-result';
function collectSaveState(){
  return {
    version:16,
    savedAt:new Date().toISOString(),
    targets:window.__selectedTargets?window.__selectedTargets():{},
    charm:{
      name:$('charmName')?.value||'',skill1:$('charmSkill1')?.value||'',lv1:+($('charmLv1')?.value||0),
      skill2:$('charmSkill2')?.value||'',lv2:+($('charmLv2')?.value||0),
      s1:+($('charmS1')?.value||0),s2:+($('charmS2')?.value||0),s3:+($('charmS3')?.value||0)
    },
    weaponSlots:[1,2,3].map(i=>+($('weaponSlot'+i)?.value||0)),
    tier:$('tier')?.value||'mr',allowQurious:!!$('allowQurious')?.checked,limit:$('limit')?.value||'10'
  };
}
function applySaveState(x){
  if(!x)return;
  const c=x.charm||{};
  const set=(id,v)=>{const e=$(id);if(e&&v!==undefined)e.value=v};
  set('charmName',c.name);set('charmSkill1',c.skill1);set('charmLv1',c.lv1);set('charmSkill2',c.skill2);set('charmLv2',c.lv2);
  set('charmS1',c.s1);set('charmS2',c.s2);set('charmS3',c.s3);
  (x.weaponSlots||[]).forEach((v,i)=>set('weaponSlot'+(i+1),v));
  set('tier',x.tier||'mr'); if($('allowQurious'))$('allowQurious').checked=x.allowQurious!==false; set('limit',x.limit||'10');
  if(window.__restoreTargets) window.__restoreTargets(x.targets||{});
}
function saveCurrent(){
  try{localStorage.setItem(SAVE_KEY,JSON.stringify(collectSaveState()));return true}catch(e){console.warn('save failed',e);return false}
}
function loadCurrent(){try{const raw=localStorage.getItem(SAVE_KEY);if(!raw)return false;applySaveState(JSON.parse(raw));return true}catch(e){console.warn('load failed',e);return false}}
function saveLastResult(results,t,nodes,pools,note){
  try{localStorage.setItem(RESULT_KEY,JSON.stringify({version:16,savedAt:new Date().toISOString(),state:collectSaveState(),results,targets:t,nodes,pools:pools.map(x=>x.length),note}))}catch(e){console.warn('result cache failed',e)}
}
function loadLastResult(){try{const raw=localStorage.getItem(RESULT_KEY);return raw?JSON.parse(raw):null}catch(e){return null}}
function wirePersistence(){
  $('saveBtn')?.addEventListener('click',()=>{const ok=saveCurrent();const st=$('status');if(st){st.className='card ok';st.innerHTML=ok?'<b>設定を保存しました</b><p class="small">このブラウザのキャッシュに保存されています。</p>':'<b>保存できませんでした</b>';st.classList.remove('hidden')}});
  $('loadBtn')?.addEventListener('click',()=>{const ok=loadCurrent();const st=$('status');if(st){st.className='card '+(ok?'ok':'warn');st.innerHTML=ok?'<b>保存した設定を復元しました</b>':'<b>保存された設定がありません</b>';st.classList.remove('hidden')}});
  $('clearSaveBtn')?.addEventListener('click',()=>{localStorage.removeItem(SAVE_KEY);localStorage.removeItem(RESULT_KEY);const st=$('status');if(st){st.className='card ok';st.innerHTML='<b>保存データを消去しました</b>';st.classList.remove('hidden')}});
}
