const API='https://wilds.mhdb.io/en';
...
async function loadDB(){try{const [armor,skills,deco,charms]=await Promise.all(['armor','skills','decorations','charms'].map(k=>fetch(API+'/'+k).then(r=>r.json())));W.armor=armor;W.skills=skills;W.decorations=deco;W.charms=charms;W.loaded=true;$('dbVersion').textContent=`Wilds DB loaded / Armor ${armor.length} / Skills ${skills.length} / Decorations ${deco.length}`;renderPicker();$('solveBtn').disabled=false;$('solveBtn').textContent='検索する';const saved=MHStorage.load('wilds');if(saved?.data){restore(saved.data);} }catch(e){$('dbVersion').textContent='Wilds DBの読み込みに失敗しました';$('status').classList.remove('hidden');$('status').textContent='DB接続エラー: '+e.message;}}
