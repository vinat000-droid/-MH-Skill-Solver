...
function flattenCharms(){
  const out=[{id:'none',name:'護石なし',skills:[],source:null}];
  for(const c of W.charms){if(c?.randomized)continue;for(const r of (c.ranks||[])){if(!(r.skills||[]).length)continue;out.push({id:`${c.id}:${r.id}`,name:r.name||c.name||`護石${c.id}`,skills:r.skills,source:c,rank:r});}}
  // MHDB intentionally exposes randomized charms without rolled skills.  Add only
  // skill/level combinations explicitly verified in the separate random-charm rule table.
  for(const r of (W.randomCharmRules||[])){
    const sk=W.skills.find(x=>norm(x?.name)===norm(r.name));
    if(!sk)continue;
    const max=Math.min(Number(r.maxLevel)||1,skillMaxLevel(sk));
    for(let lv=1;lv<=max;lv++){
      out.push({id:`random:${r.name}:${lv}`,name:`鑑定護石（ランダム） ${r.name} Lv${lv}`,skills:[{skill:{id:sk.id,name:sk.name},level:lv}],source:{randomized:true,rule:r},randomized:true,randomRule:r});
    }
  }
  return out;
}
...
async function loadDB(){try{const [armor,armorSets,weapons,skills,deco,charms,randomCharmRules]=await Promise.all(['/armor','/armor/sets','/weapons','/skills','/decorations','/charms'].map(fetchJSON).concat([fetch('./data/random-charm-rules.json').then(r=>{if(!r.ok)throw new Error('random-charm-rules.json '+r.status);return r.json();}).catch(()=>[])]));W.armor=armor;W.armorSets=armorSets;W.weapons=weapons;W.skills=skills;W.decorations=deco;W.charms=charms;W.randomCharmRules=randomCharmRules;W.loaded=true;const audit=skillAudit();const normalCount=audit.normal;const weaponOnlyCount=audit.weaponOnly;const setCount=skills.filter(s=>s?.kind==='set').length;const groupCount=skills.filter(s=>s?.kind==='group').length;const randomCharmRuleCount=(randomCharmRules||[]).length;$('dbVersion').textContent=`Wilds 日本語DB loaded / Armor ${armor.length} / Weapons ${weapons.length} / Skills ${skills.length} / 通常候補 ${normalCount} / 武器限定 ${weaponOnlyCount} / Series ${setCount} / Group ${groupCount} / 未供給 ${audit.orphan.length} / Decorations ${deco.length} / Charms ${charms.length}`;renderPickers();$('solveBtn').disabled=false;$('solveBtn').textContent='検索する';const saved=MHStorage.load('wilds');if(saved?.data)restore(saved.data);}catch(e){$('dbVersion').textContent='Wilds 日本語DBの読み込みに失敗しました';$('status').classList.remove('hidden');$('status').textContent='DB接続エラー: '+e.message;}}
